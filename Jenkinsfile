    ...
    stages{
      ...
      steps {
        ...
        stage('Containerize And Test') {
            steps {
                script{
                    bat 'docker run -d  --name customize-service -e FLASK_APP=run.py awjeet777/newsread-customize && timeout /t 10 >nul && docker logs customize-service && docker stop customize-service'
                    bat 'docker run -d  --name news-service -e FLASK_APP=run.py awjeet777/newsread-news && timeout /t 10 >nul && docker logs news-service && docker stop news-service'
                }
            }
        }
